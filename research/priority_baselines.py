...
PRIORITY_SCENARIO_ORDER = ("one", "two-g", "two-2g")
...
def plot_results(
    ...
    if not summary:
        return
    scenario_names = {str(row["priority_scenario"]) for row in summary}
    scenarios = [name for name in PRIORITY_SCENARIO_ORDER if name in scenario_names]
    scenarios.extend(sorted(scenario_names - set(scenarios)))
    ...
    speed_max = max(
        float(row[metric])
        for row in summary
        for metric in ("mean_speed_all_mean", "mean_speed_priority_mean")
    )
    speed_ylim = (0.0, max(1.0, 1.05 * speed_max))
    for axis in axes[:, 1:3].flat:
        axis.set_ylim(speed_ylim)
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(path)
    plt.close(fig)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--plot-only",
        action="store_true",
        help="rebuild the PNG from <out-dir>/<prefix>.json without simulations",
    )
    ...
    if args.plot_only and args.no_plot:
        raise ValueError("--plot-only cannot be combined with --no-plot")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.plot_only:
        artifact_path = args.out_dir / f"{args.prefix}.json"
        plot_path = args.out_dir / f"{args.prefix}.png"
        if not artifact_path.is_file():
            raise FileNotFoundError(f"plot artifact not found: {artifact_path}")
        payload = json.loads(artifact_path.read_text(encoding="utf-8"))
        summary = payload.get("summary")
        paired_aggregate = payload.get("paired_summary")
        if not isinstance(summary, list) or not isinstance(paired_aggregate, list):
            raise ValueError(
                f"plot artifact must contain summary and paired_summary lists: "
                f"{artifact_path}"
            )
        plot_results(plot_path, summary, paired_aggregate)
        print(f"Rebuilt plot from {artifact_path}")
