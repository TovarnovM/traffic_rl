...
def test_occupied_adjacent_cell_no_crash_and_flags():
    ...
    assert obs.action_mask[0, 2] == False
...
def test_simulator_observe_matches_builder_and_safe_mask_alignment():
    ...
    i_left_safe = LOCAL_OBSERVATION_FEATURES.index("left_safe")
    i_right_safe = LOCAL_OBSERVATION_FEATURES.index("right_safe")
    for r in range(via.obs.shape[0]):
        assert bool(via.action_mask[r, 0]) == bool(via.obs[r, i_left_safe])
        assert bool(via.action_mask[r, 2]) == bool(via.obs[r, i_right_safe])
